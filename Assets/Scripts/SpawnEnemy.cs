...
namespace Kuoan
{
    ...
    public class SpawnEnemy : MonoBehaviour
    {
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                //繪製球體(座標，半徑)
                Gizmos.DrawSphere(transform.position + spawnEnemyDatas[i].point, 0.2f);
            }
        }
        ...
        private void Spawn()
        {
            for (int i = 0; i < spawnEnemyDatas.Length; i++)
            {
                GameObject temp = Instantiate(prefabEnemy, transform.position + spawnEnemyDatas[i].point, Quaternion.identity);
                temp.GetComponent<ControlSystemEnemy>().SetWeaponType(spawnEnemyDatas[i].type);
            }
        }
    }
